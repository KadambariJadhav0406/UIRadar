...
const paymentPayload = {
  PackageId: location.state?.PackageId, // from previous page
  Amount: Number(location.state?.Amount), // use amount passed via location.state
...
const handleSubmit = async (event) => {
  ...
  try {
    ...
    const response = await axios.post(
      "http://localhost:5050/makePayment",
      paymentPayload,
      config
    );
    ...
    if (response.status === 200) {
      ...
      setTimeout(() => navigate("/createrequest"), 1500);
    } else {
